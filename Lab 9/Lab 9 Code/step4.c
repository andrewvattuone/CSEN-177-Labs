...
/*Description:
    Reads 1 byte sizeof(buffer) times from a file at a time, and stores it in a buffer. Immediately afterward it writes
    the bytes stored in buffer into a new file. Buffer size is still determined by command line input. Later on the time it takes to do this 
